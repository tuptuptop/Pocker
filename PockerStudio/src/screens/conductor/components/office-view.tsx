/**
 * Animated SVG office view — shows agent desks, monitors, speech bubbles,
 * status glows, social spots, and wandering idle agents.
 *
 * Three layouts: Grid, Roundtable, War Room.
 * Pure CSS animations + SVG, no framer-motion.
 *
 * Ported from upstream hermes-workspace office-view.tsx with adaptations:
 * - localStorage key: hermes-studio:office-layout
 * - Header text: Mission Control
 * - Imports AgentAvatar / AGENT_ACCENT_COLORS from ./agent-avatar
 * - Exports AgentWorkingRow and AgentWorkingStatus types
 */
...
export function OfficeView({
  ...
  const [layoutTemplate, setLayoutTemplate] = useState<OfficeLayoutTemplate>(() => {
    ...
    const saved = window.localStorage.getItem('hermes-studio:office-layout')
  ...
  const changeLayout = (nextTemplate: OfficeLayoutTemplate) => {
    ...
    if (typeof window !== 'undefined') {
      window.localStorage.setItem('hermes-studio:office-layout', nextTemplate)
    }
  }
